...
-- 성적테이블에서 각 과목의 총점과 평균 계산하여 계산하기
...
-- city table 에서 인구(population)가 1만 이상인 도시들만 찾으시오 
...
먼저 국가별 인구평균을 계산하고 
계산된  인구평균이 5만 이상인 경우 
avg() 함수로 계산 결과에 조건을 부여하기 때문에 
이러한 경우는 WHERE 가 아니고, HAVING 절을 GROUP BY 다음에 둔다  
*/

SELECT CountryCode, AVG(population) AS 평균인구수
FROM city 
...
max() 함수는 각 그룹에서 최대값을 찾는 함수이다
이 함수를 사용할 때 특이한 점이
한 개의 칼럼(name,도시명) 을 GROUP BY 로 묶지않고 코드를 실행하면 
인구가 가장 많은 도시의 이름을 알 수 있다
*/
...
인구가 1만 이상 5만 이하인 도시의 국가 이름이 무엇인가 조회하라 
*/
