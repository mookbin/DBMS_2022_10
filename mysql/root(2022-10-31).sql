...
-- world SCHEMA 를 대상으로 DB를 핸들링(CRUD)하기 위하여 
-- world SCHEMA open 
USE world;
/*
country table 에서
각 국가별 GNP 값이 큰 도시부터 리스트 조회 
단, GNP 1000 이상인 도시  
*/
...
인구가 1만 이상 5만 이하인 도시의 국가 정보를 같이 확인하고싶다.
*/
-- city table 에서 인구가 5만 이상인 국가들을 표현 
-- 여기는 CountryCode 가 있는데 이 코드가 무엇을 의미하는지 
-- 알기 어렵다


-- countryColde 가 무엇을 의미하는지를 살펴봤더니
...
-- 완전 JOIN(EQ JOIN)
-- 완전 JOIN 주의사항
-- city table CountryCode 데이터가 "반드시" Country 테이블의 code 데이터에 존재해야한다
...
/*
두 table 간에 "참조무결성" 조건이 없거나 
"참조 무결성" 성립이 아직 이루어 지지 않았을 경우 
예를 들어 성적 table 과 학생 table 을 JOIN 하려고 하는데 
성적 table 에는 특정 학번의 성적이 추가되어 있으나
학생 table 에는 특정 학번의 학생 정보가 아직 추가되지 않았을 경우 
EQ JOIN 을 수행하면 성적 데이터가 보이지 않게 된다. 

두 (성적, 학생정보)table 을 JOIN 했을 때
성적 table 의 모든 데이터는 일단 보여주고 
학생정보가 있으면 학생 정보도 같이 보여주고 
없으면 다른 표식으로 보여주면 좋곘다 
이럴 때 사용하는 JOIN 을 outer JOIN 이라고 한다 
*/

-- 1. city table WHERE 조건에 맞도록 SELECT 하고 
-- 2. SELECT 된 List 에서 CountryCode 데이터를 가지고 
-- 3. country table에서 SELECT 를 수행한다
-- 4. country table 에서 데이터가 SELECT 괴면 해당 데이터를 보여주고 
-- 5. 없으면 <null> 이라고 보여준다
