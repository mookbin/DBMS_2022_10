-- 여기는 root 접속화면
/*
root 권한은 DBA(DataBase Administor) 역할을 수행하며
...
/*
SQL : 대부분의 RDMS 데이터 관리 소프트웨어에서 공통으로 
통용되는 Data 관련 언어
DDL, DML, DCL, TCL
...
-- 현재 MYSQL System 에 어떤 Schema 들이 있냐?
...
-- 아래코드는 st_num 값만 있고, st_name 값이 없는 SQL 이다
...
-- S0003 인 임꺽정 데이터를 추가하려고 했더니 학번이 중복되었다는 
-- 오류가 났다 
-- 임꺽정의 학번은 분명 S0003 이 맞다. 그렇다면 이전에 입력된 학번이 
-- 잘못 입력되었다는 것이다
-- S0003 학번의 학생이 누구인지 찾아라 
-- 찾았더니 성춘향 학생의 학번이 S0003 으로 저장되어 있다
-- 원본 데이터를 확인해 보니 성춘향 학생의 학번이 S0033 이더라 
-- 첫번째 방법 : 성춘향 학생의 데이터를 삭제하고 임꺽정 학생 데이터 추가
